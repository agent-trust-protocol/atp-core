...
export class MCPIntegratedAgent extends BaseAgent {
  ...
  private delegatedTools: Map<string, { tool: MCPTool; delegatedBy: string; expires: Date }> = new Map();

  constructor(name: string, mcpEnabled: boolean = true) {
  ...
  async establishMCPSession(targetAgent: string): Promise<MCPSession> {
  ...
  private deriveMCPPermissions(trustLevel: string): string[] {
  ...
  private getAuthorizedTools(trustLevel: string): MCPTool[] {
  ...
  async delegateMCPTool(toolName: string, targetAgent: string, duration: number = 3600000): Promise<boolean> {
  ...
  async requestMCPCapability(targetAgent: string, capability: string): Promise<MCPTool[]> {
  ...
  async executeDelegatedTool(toolName: string, params: any, delegationToken: string): Promise<any> {
  ...
  private async handleMCPSessionEstablishment(params: any, id: string): Promise<void> {
  ...
  private async handleMCPToolDelegation(params: any, id: string): Promise<void> {
  ...
  private async handleMCPToolExecution(params: any, id: string): Promise<void> {
  ...
  private async handleMCPCapabilityRequest(params: any, id: string): Promise<void> {
