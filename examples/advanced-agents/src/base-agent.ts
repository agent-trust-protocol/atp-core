...
export class BaseAgent {
  ...
  public credentials: AgentCredential[] = [];
  public trustNetwork: Map<string, TrustRelationship> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error(`❌ Failed to initialize agent ${this.config.name}:`, (error as Error).message);
      throw error;
    }
  }
  ...
  async sendRequest(method: string, params: any = {}, targetAgent?: string): Promise<any> {
    if (!this.ws) {
  ...
  async sendError(id: string, error: { code: number; message: string }): Promise<void> {
    if (!this.ws) return;
  ...
  async establishTrust(targetAgentDid: string): Promise<TrustRelationship> {
    ...
    try {
      ...
      console.error(`❌ Failed to establish trust with ${targetAgentDid}:`, (error as Error).message);
      throw error;
    }
  }

  private async verifyAgentCredentials(agentDid: string, credentials: AgentCredential[]): Promise<'unknown' | 'basic' | 'verified' | 'trusted'> {
    ...
    if (hasHighTrustCredentials) return 'trusted';
    if (hasValidCredentials) return 'verified';
    return 'basic';
  }

  async requestCapability(targetAgentDid: string, capability: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`❌ Error requesting capability:`, (error as Error).message);
      return false;
    }
  }

  // Message handlers
  private async handleTrustHandshake(params: any, id: string): Promise<void> {
  ...
  private async handleCapabilityRequest(params: any, id: string): Promise<void> {
  ...
  private async handleCredentialVerification(params: any, id: string): Promise<void> {
  ...
  private async handleCollaborationInvite(params: any, id: string): Promise<void> {
  ...
  private async handleToolDelegation(params: any, id: string): Promise<void> {
