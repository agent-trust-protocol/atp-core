...
export class DIDCertificateAuthority {
  ...
  private async initializeCA(): Promise<void> {
    ...
    const signature = await ATPEncryptionService.sign(
      JSON.stringify(caCertData),
      this.caPrivateKey
    );
  ...
  async verifyCertificate(certificate: DIDCertificate): Promise<{
    valid: boolean;
    reason?: string;
    trustLevel?: TrustLevel;
  }> {
  ...
  async getCertificate(certificateId: string): Promise<DIDCertificate | null> {
    return this.certificates.get(certificateId) || null;
  }

  async getCertificateByDID(did: string): Promise<DIDCertificate[]> {
    return Array.from(this.certificates.values()).filter(
      cert => cert.subjectDID === did && cert.status === 'active'
    );
  }
  ...
  private async verifyCertificateRequest(request: CertificateRequest): Promise<void> {
    // Verify proof of possession of private key
    const isValidProof = await ATPEncryptionService.verify(
  ...
  private getExtendedKeyUsage(trustLevel: TrustLevel): string[] {
    switch (trustLevel) {
