    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enterprise compliance metrics and status</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(metrics.complianceMetrics).map(([compliance, score]) => (
              <div key={compliance} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{compliance.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</span>
                  <Badge variant={score > 95 ? 'default' : score > 90 ? 'secondary' : 'destructive'}>
                    {score}%
                  </Badge>
                </div>
